 ...
 */
- (FFBRequest*)openUrl:(NSString *)url
               params:(NSMutableDictionary *)params
           httpMethod:(NSString *)httpMethod
             delegate:(id<FBRequestDelegate>)delegate {

    NSMutableDictionary *contentDictionary = [[NSMutableDictionary alloc]init];
    [contentDictionary setValue:@"EVERYONE" forKey:@"value"];
  ...
  [params setValue:jsonStr forKey:@"privacy"];
  if ([self isSessionValid]) {
    [params setValue:self.accessToken forKey:@"access_token"];
  }

  //orlyn [_request release];
  /*orlyn 
   ...
   */
    _request = [FFBRequest getRequestWithParams:params
                                     httpMethod:httpMethod
                                       delegate:delegate
                                    requestURL:url];
  [_request connect];
...
- (void)authorizeWithFBAppAuth:(BOOL)tryFBAppAuth
  ...
  if ([device respondsToSelector:@selector(isMultitaskingSupported)] && [device isMultitaskingSupported]) {
    ...
    if (tryFBAppAuth) {
        NSLog(@"FB OPTION 1 - Authorize against the app, if installed");
        NSString *fbAppUrl = [FFBRequest serializeURL:kFBAppAuthURL params:params];
        NSLog(@"%@",fbAppUrl);
        didOpenOtherApp = [[UIApplication sharedApplication] openURL:[NSURL URLWithString:fbAppUrl]];
    }

    if (trySafariAuth && !didOpenOtherApp) {
        NSLog(@"FB OPTION 2 - Look for a cookie in safari");
        NSString *nextUrl = [NSString stringWithFormat:@"fb%@://authorize", _appId];
        [params setValue:nextUrl forKey:@"redirect_uri"];
        NSString *fbAppUrl = [FFBRequest serializeURL:loginDialogURL params:params];
...
- (NSDictionary*)parseURLParams:(NSString *)query {
	...
	//orlyn NSMutableDictionary *params = [[[NSMutableDictionary alloc] init] autorelease];
...
- (void)authorize:(NSArray *)permissions
  ...
  //orlyn [_permissions release];
 //orlyn _permissions = [permissions retain];
...
- (BOOL)handleOpenURL:(NSURL *)url {
  ...
  if (![[url absoluteString] hasPrefix:[NSString stringWithFormat:@"fb%@://authorize", _appId]]) {
      NSLog(@"looks like from app returning no");
    return NO;
  }
...
- (void)logout:(id<FBSessionDelegate>)delegate {
  ...
  //orlyn [params release];
 // [_accessToken release];
  _accessToken = nil;
  //orlyn [_expirationDate release];
  _expirationDate = nil;
 ...
 */
- (FFBRequest*)requestWithParams:(NSMutableDictionary *)params
 ...
 *            Returns a pointer to the FBRequest object.
 */
- (FFBRequest*)requestWithMethodName:(NSString *)methodName
 ...
 *            Returns a pointer to the FBRequest object.
 */
- (FFBRequest*)requestWithGraphPath:(NSString *)graphPath
                 andDelegate:(id <FBRequestDelegate>)delegate {
